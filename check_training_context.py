...
# Expected format based on batch_generation.py:
# data = [train_input, train_target, cv_input, cv_target, train_history, cv_history, 
                    ...
                    if non_empty <= 3:
                        ...
                        if len(ctx) > 0 and ctx[0] is not None:
                            if hasattr(ctx[0], 'shape'):
                                ...
                                if ctx[0].numel() > 0:
                                    print(f"      First detection: {ctx[0][0] if len(ctx[0]) > 0 else 'empty'}")
...
if len(data) >= 9:
    ...
    if cv_context is not None:
        ...
        if isinstance(cv_context, list):
            ...
            non_empty = 0
            for i, ctx in enumerate(cv_context):
                if ctx is not None and len(ctx) > 0:
                    non_empty += 1
...
# Check what kind of data is in there
print("\n" + "="*80)
print("Inspecting first few elements of data tuple:")
print("="*80)
